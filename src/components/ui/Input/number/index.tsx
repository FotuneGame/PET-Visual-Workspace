...
const NumberInput:FC<IProps> = React.memo(({label,placeholder,value,callback,disabled,min,max,step})=>{
    ...
    return(
        <div className={[style.wrapper,theme==='light' ? style.light : style.dark, disabled ? style.disabled : ""].join(" ")}>
            <label>{label}</label>
            <input placeholder={placeholder} max={range.maximum} min={range.minimum} step={step} type={"number"} value={value} onChange={valid} disabled={disabled}/>
        </div>
    )
});
